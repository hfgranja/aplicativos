...
func NewRouter(
	...
	pyramidService := services.NewPyramidService(orm)
	...
	privateGroup.Use(authMiddleware.AuthTokenRequired)
	{
		...
		setPyramidGroup(privateGroup, pyramidService)
	}
...
func setPyramidGroup(parent *gin.RouterGroup, svc *services.PyramidService) {
	// Tenants (multi-tenancy)
	tenantsGroup := parent.Group("/pyramid/tenants")
	{
		tenantsGroup.GET("/", svc.GetTenants)
		tenantsGroup.POST("/", svc.CreateTenant)
	}

	// Test plans
	plansGroup := parent.Group("/pyramid/plans")
	{
		plansGroup.GET("/", svc.GetTestPlans)
		plansGroup.POST("/", svc.CreateTestPlan)
		plansGroup.GET("/:plan_id", svc.GetTestPlan)
		plansGroup.GET("/:plan_id/summary", svc.GetPyramidSummary)
	...
	// Findings
	findingsGroup := parent.Group("/pyramid/findings")
	{
	...
	// Release policies
	policiesGroup := parent.Group("/pyramid/policies")
	{
		policiesGroup.GET("/", svc.GetReleasePolicies)
		policiesGroup.POST("/", svc.CreateReleasePolicy)
	}

	// Audit trail
	auditGroup := parent.Group("/pyramid/audit")
	{
		auditGroup.GET("/", svc.GetAuditEvents)
	}

	// Corpus cases
	corpusGroup := parent.Group("/pyramid/corpus")
	{
		corpusGroup.GET("/", svc.GetCorpusCases)
		corpusGroup.POST("/", svc.CreateCorpusCase)
	}
}
